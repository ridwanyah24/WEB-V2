          ...
          <div className="rounded-lg shadow-sm mx-auto overflow-hidden mb-6">
            <div className="p-6">
              {records.map((record, recordIndex) => {
                ...
                // Use the right meanings depending on language
                const meanings = isEnglish
                  ? record.meanings
                  : record.translations?.urh || [];

                return (
                  <div key={recordIndex} className="mb-12">
                    {meanings?.map((meaning: any, meaningIndex: number) => {
                      // pick opposite side as translation
                      const translation = isEnglish
                        ? record.translations?.urh?.[meaningIndex]
                        : record.meanings?.[meaningIndex];

                      return (
                        <div key={meaningIndex} className="mb-10">
                          {/* Header */}
                          <div className="flex lg:gap-5 gap-5 items-start mb-4">
                            <span className="mr-2 bg-[#05353a30] px-4 py-2 text-[#05353a] font-bold rounded-full">
                              {meaningIndex + 1}
                            </span>
                            <div className="mt-2">
                              ...
                              {meaning.definitions?.length > 0 && (
                                <div className="mt-5">
                                  ...
                                  {meaning.definitions.map(
                                    (definition: string, defIndex: number) => (
                                ...
                                <div className="mt-5">
                                  ...
                                  {meaning.examples.map(
                                    (example: string, exIndex: number) => (
                                      <div key={exIndex} className="mb-2 mt-5">
